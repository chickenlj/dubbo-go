...
func (tp *TripleProtocol) Export(invoker protocol.Invoker) protocol.Exporter {
	...
	var info *common_pro.ServiceInfo
	infoRaw, ok := url.GetAttribute(constant.ServiceInfoKey)
	if ok {
		info = infoRaw.(*common_pro.ServiceInfo)
	}
...
func (tp *TripleProtocol) exportForTest(invoker protocol.Invoker, info *common_pro.ServiceInfo) protocol.Exporter {
	url := invoker.GetURL()
	serviceKey := url.ServiceKey()
...
func (tp *TripleProtocol) openServer(invoker protocol.Invoker, info *common_pro.ServiceInfo) {
	url := invoker.GetURL()
	tp.serverLock.Lock()
